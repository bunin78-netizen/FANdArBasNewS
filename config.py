...
def promo_caption() -> str:
    return (
        f"FUNDARBAS — платформа для арбитража и Basis в одном окне 🚀\n\n"
        f"Если вы торгуете на нескольких биржах, знаете, сколько времени уходит на ручной мониторинг и синхронное открытие ног.\n"
        f"FUNDARBAS автоматизирует этот процесс:\n\n"
        f"✅ Сканирует спреды и funding-rate\n"
        f"✅ Фильтрует входы по вашим правилам\n"
        f"✅ Открывает и сопровождает сделки\n"
        f"✅ Показывает PnL, ошибки и состояние бирж\n"
        f"✅ Использует AI Gatekeeper для отсечки слабых/рискованных входов\n\n"
        f"Подходит трейдерам, которые хотят меньше рутины и больше контроля над системной торговлей.\n\n"
        f"Если нужен доступ к демо/подключению — пишите в ЛС.\n\n"
        f"Торговля с плечом связана с риском. Не является инвестрекомендацией.\n\n"
        f"👉 {PROMO_LINK}"
    )
